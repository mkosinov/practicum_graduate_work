...
class Alisa:
    def __init__(self, request, response):
        self.event, self.answer, self.response = (
            request,
            response["response"],
            response,
        )
        self.intents = (
            self.event.get("request", {}).get("nlu", {}).get("intents", {})
        )
        self.command = self.event.get("request", {}).get("command")
        self.state_session = self.event.get("state", {}).get("session", {})
        self.application_state = self.event.get("state", {}).get(
            "application", {}
        )
    ...
    def update_user_state(self, key, value):
        self.response["user_state_update"] = self.response.get(
            "user_state_update", {}
        )
    ...
    def add_transition(self, name, context, handler):
        self.response["session_state"] = self.response.get("session_state", {})
        self.response["session_state"]["__transitions__"] = self.response.get(
            "session_state", {}
        ).get("__transitions__", [])
    ...
    def show_episode(
        ...
        show_item_meta = {}
        show_item_meta["content_id"] = (
            episode_id if episode_id is not None else str(date.today())
        )
        if title:
        ...
        show_item_meta["publication_date"] = (
            pub_date
